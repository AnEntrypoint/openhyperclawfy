...
fastify.post('/api/avatar/upload', async (req, reply) => {
  const mp = await req.file()
  if (!mp) {
  ...
  if (ext !== 'vrm') {
    return reply.code(400).send({ error: 'File must have a .vrm extension' })
  }
  ...
  // Validate size (25MB default)
  const maxSize = parseInt(process.env.MAX_VRM_UPLOAD_SIZE || '25', 10) * 1024 * 1024
  if (buffer.length > maxSize) {
    return reply.code(400).send({ error: `File exceeds max size of ${maxSize / (1024 * 1024)}MB` })
  }

  // Validate GLB magic bytes (VRM is a GLB container)
  if (buffer.length < 12) {
    return reply.code(400).send({ error: 'File too small to be a valid VRM' })
  }
  const magic = buffer.readUInt32LE(0)
  if (magic !== 0x46546C67) { // 'glTF'
    return reply.code(400).send({ error: 'Invalid VRM file: missing glTF magic bytes' })
  }
  const version = buffer.readUInt32LE(4)
  if (version !== 2) {
    return reply.code(400).send({ error: 'Invalid VRM file: must be glTF version 2' })
  }
  ...
  // Convert to File and upload via existing pipeline
  const file = new File([buffer], filename, {
    type: 'model/gltf-binary',
  })
  await assets.upload(file)

  const url = `${assets.url}/${filename}`
  return { hash, url }
})
