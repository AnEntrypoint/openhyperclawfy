...
function randomSpawnPosition(center) {
  const angle = Math.random() * Math.PI * 2
  const radius = 0.5 + Math.random() * (SPAWN_RADIUS - 0.5)
  return [
...
export class ServerNetwork extends System {
  ...
  async onConnection(ws, params) {
    try {
      ...
      // spectator mode: no player entity, just observe
      if (mode === 'spectator') {
        socket.player = null
      ...
      socket.player = this.world.entities.add(
        {
          id: user.id,
          type: 'player',
          position: randomSpawnPosition(this.spawn.position),
