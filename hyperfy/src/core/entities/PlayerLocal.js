...
export class PlayerLocal extends Entity {
  ...
  update(delta) {
    ...
    // rotate direction to face camera Y direction (or simulated yaw for agents)
    if (xr) {
      this.world.camera.getWorldQuaternion(q1)
      ...
      const lookYaw = this.world.controls._simulatedYaw !== null
        ? this.world.controls._simulatedYaw
        : this.cam.rotation.y
      const yQuaternion = q1.setFromAxisAngle(UP, lookYaw)
      this.moveDir.applyQuaternion(yQuaternion)
    }
    ...
    if (xr) {
      ...
      // explicit look direction set by agent (simulateLook)
      rotY = this.world.controls._simulatedYaw
    } else if (this.moving && !this.control.pointer.locked) {
      // no explicit look and no mouse input: face movement direction
