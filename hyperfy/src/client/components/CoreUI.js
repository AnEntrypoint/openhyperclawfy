...
export function CoreUI({ world }) {
  ...
  useEffect(() => {
    ...
    const onReady = () => {
      setIsSpectator(world.network.isSpectator)
    }
  ...
  return (
    ...
    >
      ...
      {!ui.reticleSuppressors && !isSpectator && <Reticle world={world} />}
      {<Toast world={world} />}
      {ready && !isSpectator && <ActionsBlock world={world} />}
      {ready && !isSpectator && <Sidebar world={world} ui={ui} />}
      {ready && <Chat world={world} />}
      {/* {ready && <Side world={world} player={player} menu={menu} />} */}
      {avatar && !isSpectator && <AvatarPane key={avatar.hash} world={world} info={avatar} />}
...
function SpectatorHUD({ world }) {
  const [visible, setVisible] = useState(true)
  const [info, setInfo] = useState(() => world.spectatorInfo || {
    mode: 'agentFocus',
  })
  useEffect(() => {
    world.on('spectator-mode', setInfo)
    const onToggle = () => setVisible(v => !v)
    world.on('spectator-hud-toggle', onToggle)
    return () => {
      world.off('spectator-mode', setInfo)
      world.off('spectator-hud-toggle', onToggle)
    }
  }, [])
  ...
  return (
    ...
    >
      <div className='spec-keys'>
        <div className='spec-key'><kbd>C</kbd> {isAgentFocus ? 'Freecam' : 'Agent Focus'}</div>
        {isAgentFocus && <div className='spec-key'><kbd>LMB</kbd><kbd>RMB</kbd> Cycle agents</div>}
        {isAgentFocus && <div className='spec-key'><kbd>Scroll</kbd> Zoom</div>}
        {!isAgentFocus && <div className='spec-key'><kbd>WASD</kbd> Move</div>}
        {!isAgentFocus && <div className='spec-key'><kbd>Q</kbd><kbd>E</kbd> Up/Down</div>}
        {!isAgentFocus && <div className='spec-key'><kbd>Shift</kbd> Fast</div>}
        <div className='spec-key'><kbd>Click</kbd> Mouse look</div>
        <div className='spec-key'><kbd>~</kbd> Toggle HUD</div>
      </div>
    </div>
  )
}
